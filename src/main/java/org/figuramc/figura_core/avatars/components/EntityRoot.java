...
public class EntityRoot implements AvatarComponent<EntityRoot> {
    ...
    public static final Type<EntityRoot> TYPE = new Type<>("ENTITY_ROOT", EntityRoot::new, RenderDataHolder.TYPE, Textures.TYPE, Materials.TYPE, Molang.TYPE, VanillaRendering.TYPE);
    ...
    public EntityRoot(Avatar<?> avatar, AvatarModules modules) throws AvatarInitError, AvatarOutOfMemoryError {
        ...
        for (AvatarModules.LoadTimeModule mod : modules.loadTimeModules()) {
            ...
            FiguraModelPart part = new FiguraModelPart(avatar, "", mod, mod.materials.entityRoot());
        ...
        root.getTransform().setScale(1f / 16); // Scale by 1/16 for entity!
        // Make sure we build rendering data for this root and save it in the holder
