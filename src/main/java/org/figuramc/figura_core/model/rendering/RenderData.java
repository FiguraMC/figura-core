...
public class RenderData implements AutoCloseable {
    ...
    private RenderData(Builder built, @Nullable RenderDataHolder holder, @Nullable AllocationTracker<AvatarOutOfMemoryError> allocationTracker) throws AvatarOutOfMemoryError {
        ...
        var byPriority = built.buildingBuffers.entrySet().stream().collect(Collectors.groupingBy(e -> e.getKey().priority(), TreeMap::new, Collectors.toList()));
        ...
        if (holder != null) holder.register(this);
    }
...
//        private float curY, curLineHeight;
...
//        public void addText(FormattedText text) throws AvatarError {
//            // Set up initial variables
//            defaultLineHeight = FiguraConnectionPoint.GLYPH_PROVIDER.getDefaultLineHeight();
//            curY = 0; curLineHeight = 0;
...
//        private int processText(FormattedText text, int charIndex) throws AvatarError {
//            style = text.style;
...
//        }
//
//        private void processChar(int codepoint, int charIndex) throws AvatarError {
...
//                flushLine();
//            } else {
//                boolean obfuscated = style.obfuscated.value(charIndex);
...
//                float width = (info.width() + (bold ? 1 : 0)) * scale.x;
//                curLine.add(new PendingChar(info, bold, scale.x, scale.y, width, glyphHeight, style, charIndex));
//            }
//        }
//
//        private void flushLine() throws AvatarError {
...
//        }
//
//        // Cache the render types
//        private static final Map<FiguraRenderType.TextureBinding, FiguraRenderType> TEXT_RENDERTYPES = new HashMap<>();
//
//        private void addChar(float x, PendingChar pendingChar) throws AvatarError {
//            FiguraRenderType renderType = pendingChar.info.textureBinding()
//            ByteBufferBuilder buffer = buildingBuffers.computeIfAbsent(renderType.drawCallInfo(), x -> new ByteBufferBuilder());
//
//        }
//
//
//        private record PendingChar(MinecraftGlyphProvider.GlyphInfo info, boolean bold, float scaleX, float scaleY, float width, float height, TextStyle style, int charIndex) {}
