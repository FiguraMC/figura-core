...
public class MaterialAPI {
    ...
    public static FiguraRenderType _new(LuaRuntime s, LuaTable options) throws LuaError, LuaOOM {
        ...
        // Texture bindings
        List<FiguraRenderType.@Nullable TextureBinding> textureBindings = switch (options.rawget("textures")) {
            case LuaTable texturesTable -> {
                List<FiguraRenderType.TextureBinding> bindings = new ArrayList<>(shader.textureBindingPoints().size());
                for (int i = 0; i < shader.textureBindingPoints().size(); i++) {
                    ...
                    switch (texture) {
                        ...
                        case LuaNil nil -> bindings.add(null);
